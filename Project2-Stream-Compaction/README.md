...
Here, we can observe that the bottleneck for Work-efficient GPU stream compaction is the number of cuda Malloc we need to do when we are calling the Work-efficient scan. That is one of the unnecessary memory copy in and out which is the reason for initial bottleneck in the algoirthm with respect to the CPU one. For the case of naive, the performance is more due to the umber of threads being called and the increase in the number of additions which is of order `O(nlog(n))`.
