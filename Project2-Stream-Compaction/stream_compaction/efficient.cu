    ...
    namespace Efficient {
		...
		__global__ void kernScanDownSweep(int n, int d, int *idata) {
			...
			if (index < n - 1) {
				...
				if (index % exp == 0) {
					int t = idata[index + (exp / 2) - 1];
					idata[index + (exp / 2) - 1] = idata[index + exp - 1];
					idata[index + exp - 1] += t;
				}
			}
		}
        ...
        void scan(int n, int *odata, const int *idata) {
			...
			checkCUDAErrorFn("cudaMalloc dev_idata failed!", "efficient.cu", 86);

			// Copying the input array to the device
			cudaMemcpy(dev_idata, idataExtend, newLength * sizeof(int), cudaMemcpyHostToDevice);
			checkCUDAErrorFn("cudaMemcpy to input-copy failed!", "efficient.cu", 90);
			...
			for (int d = 0; d <= log2 - 1; d++) {
				...
				checkCUDAErrorFn("kernScanUpSweep failed!", "efficient.cu", 98);
			}
			...
			for (int d = log2 - 1; d >= 0; d--) {
				...
				checkCUDAErrorFn("kernScanDownSweep failed!", "efficient.cu", 105);
			}
			...
			checkCUDAErrorFn("cudaMemcpy to output failed!", "efficient.cu", 113);
        ...
        int compact(int n, int *odata, const int *idata) {
			...
			checkCUDAErrorFn("cudaMalloc dev_binaryMap failed!", "efficient.cu", 56);

			cudaMalloc((void**)&dev_scanResult, n * sizeof(int));
			checkCUDAErrorFn("cudaMalloc dev_scanResult failed!", "efficient.cu", 53);
			...
			// Compute temporary array od 0's and 1's
			...
			checkCUDAErrorFn("cudaMemcpy to 01-scan failed!", "efficient.cu", 81);

			scan(n, oScanData, iScanData);

			cudaMemcpy(dev_scanResult, oScanData, n * sizeof(int), cudaMemcpyHostToDevice);
			checkCUDAErrorFn("cudaMemcpy to scanResult failed!", "efficient.cu", 60);
