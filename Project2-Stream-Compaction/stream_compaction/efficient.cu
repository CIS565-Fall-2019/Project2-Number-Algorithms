...
namespace StreamCompaction {
	namespace Efficient {
		...
		int compact(int n, int *odata, const int *idata) {
			...
			checkCUDAError("cudaMalloc map_data failed!");
			...
			//mempy
			cudaMemcpy(dev_idata, idata, sizeof(int) * pow2n, cudaMemcpyHostToDevice);//host to device
			checkCUDAError("cudaMemcpy dev_idata failed!");
			//cudaMemcpy(dev_odata, odata, sizeof(int) * pow2n, cudaMemcpyHostToDevice);//host to device
			...
			StreamCompaction::Common::kernScatter << <fullBlocksPerGrid, blockSize >> > (n, dev_odata,
				dev_idata, map_data, scan_out);
			// TODO
			timer().endGpuTimer();
