...
namespace Sorting {
	namespace Radix {
		...
		__global__ void mask_generation(unsigned long long int n, int *dev_mask, int *dev_idata, int bitmask) {
			unsigned long long int index = blockDim.x * blockIdx.x + threadIdx.x;
			if (index >= n)
				return;
			dev_mask[index] = !((bool)(dev_idata[index] & bitmask));
		}
		__global__ void true_index_generation(unsigned long long int n, int *dev_t, int *dev_f, int total_falses) {
			unsigned long long int index = blockDim.x * blockIdx.x + threadIdx.x;
			if (index >= n)
				return;
			dev_t[index] = index - dev_f[index] + total_falses;
		}
		__global__ void reshuffle_mask(unsigned long long int n, int *dev_odata, int *dev_idata, int *dev_t, int *dev_f, int mask) {
			unsigned long long int index = blockDim.x * blockIdx.x + threadIdx.x;
			if (index >= n)
				return;
		...
		void sort(unsigned long long int n, int *odata, int *idata, int max_value) {
			...
			int mask = 1;
			unsigned long long int total_falses, tmp;
