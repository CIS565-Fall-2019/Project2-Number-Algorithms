...
//neural number
	...
	void forwardpass(int instance_index, float* idata,
		...
		dim3 fullBlocksPerGrid2((SIZE_OUTPUT + blockSize - 1) / blockSize);
		kernSigmoid << <fullBlocksPerGrid, blockSize >> > (SIZE_OUTPUT, dev_output, out_sig);
	...
	void compute_grad(int instance_number,
		...
		//sigmoid derivative 
		//dim3 fullBlocksPerGrid3((SIZE_OUTPUT + blockSize - 1) / blockSize);
		kernSig_partial_deriv << <fullBlocksPerGrid, blockSize >> > (SIZE_OUTPUT, out_sig, error1);
	...
	void build_network(int data_count, int num_feature, int num_class,
		...
		*/
		...
		cudaMalloc((void**)&out_sig, SIZE_OUTPUT * sizeof(float));
		checkCUDAError("cudaMalloc out_sig failed!");
	...
	void test(float* test_input, int test_times, int type) {
		...
		cudaFree(out_soft);
		cudaFree(hid_sig);
		cudaFree(weights_inandhid);
		cudaFree(weights_hidandoutput);
	}
}
