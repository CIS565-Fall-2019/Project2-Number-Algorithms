...
int main(int argc, char* argv[]) {
	...
	if (training) {
		...
		//	std::cout << layer1_adjustedWeights[i] << std::endl;
		//}
		//for (int i = 0; i < layer2_numWeights; ++i) {
		//	layer2_adjustedWeights[i] = layer2_weights[i];
		//	std::cout << layer2_adjustedWeights[i] << std::endl;
		//}

		auto start = std::chrono::steady_clock::now();
		int numRandIters = 0;
		float accumulatedError = 3.0;
		while (accumulatedError > acceptedError && numRandIters < 10000) {
			...
			/*CharacterRecognition::fillRandomWeights(layer1_numWeights, layer1_weights, numRandIters);
			CharacterRecognition::fillRandomWeights(layer2_numWeights, layer2_weights, numRandIters);*/
			for (int i = 0; i < layer1_numWeights; ++i) {
				layer1_adjustedWeights[i] = layer1_weights[i];
				std::cout << layer1_adjustedWeights[i] << std::endl;
			}
